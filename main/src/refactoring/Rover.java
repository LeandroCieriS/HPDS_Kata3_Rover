...
public class Rover {
	...
	private ArrayList<Position> obstaclesMap = new ArrayList<>();
	...
	public void go(Order... orders){
		for (Order order : orders) {
			if(order != Forward || !obstacleInFront()) {
	...
	public void setObstaclesMap(Position... positions) {
		obstaclesMap.addAll(Arrays.asList(positions));
	}

	public boolean obstacleInFront(){
		if (heading == North) return obstaclesMap.contains(new Position(position.x, position.y + 1));
		if (heading == South) return obstaclesMap.contains(new Position(position.x, position.y -1));
		if (heading == West) return obstaclesMap.contains(new Position(position.x - 1, position.y));
		return obstaclesMap.contains(new Position(position.x + 1, position.y));
	}
	...
	public static class Position {
		...
		//TODO: Refactorizar para que tengan solo un parámetro
		...
		private Position horizontalMovement(Heading heading, int sign) {
			...
			if (heading == West) return new Position(x - sign, y);
			return null;
		}
